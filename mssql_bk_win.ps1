# Windows MSSQL backup and ftp upload for Windows
# Sergei Korneev 2024
...
$pgbin = "C:\Program Files\Microsoft SQL Server\Client SDK\ODBC\170\Tools\Binn\SQLCMD.EXE"
#$pgbin = "sqlcmd.exe"
# Databases to backups

$DBnamesarray = @('zp','zp24','dh24','integra','integraold','nurli','oazisgard','oazissam','stroyka','upr22','xn24','xxt24','zplus','zpupr')

$dbprefix = ""

$BackupPath = "e:\DB_AUTOBACKUPS"
...
function CreateFtpFolder($DesFolder){
...
foreach ($dbname in $DBnamesarray) {
 ...
 Start-Process "`"$pgbin`""  -ArgumentList " -S  $dbhost  -U $dbuser -P $dbpass  -Q `"BACKUP DATABASE $dbname TO DISK=`'$BackupPath\$dbfname`' WITH INIT `" "   -Wait  -NoNewWindow 
 
 # -RedirectStandardError "$BackupPath\dump.log"

 
 if ($LASTEXITCODE -ne 0 -and $LASTEXITCODE -ne  $null  -or -not (Test-Path  "$BackupPath\$dbfname") ){
 ...
 $compress = @{
  Path = "$BackupPath\${dbfname}_${md5}"
  CompressionLevel = "Optimal"
  DestinationPath = "$BackupPath\${dbfname}_${md5}.zip"
}
Compress-Archive @compress
if (Test-Path  "$BackupPath\${dbfname}_${md5}.zip"){
   Remove-Item "$BackupPath\${dbfname}_${md5}"
}

 

 uploadToFTPServer "/$ftppath/${dbfname}_${md5}.zip" "$BackupPath\${dbfname}_${md5}.zip"
 } 
